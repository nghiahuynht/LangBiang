...
namespace WebApp.Controllers
{
    public class ConDaoController : Controller
    {
        ...
        {
            ViewBag.Ticket = ticketService.GetAllTicket() ;
            return View();
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult GetUrlPayment([FromBody] ReqPaymentDto model)
        {
            ...
            try
            {
                if (model.OrderId > 0)
                {
                    ...
                    #region ddphuong Lấy qr code chuyển khoản

   
                    string madon = Helper.GenMaDon(orderInfo.Id);
                    //QRCodePaymentModel jsbody = new QRCodePaymentModel()
                    //{
                    //    accountNo = "111002936272",
                    //    accountName = "Trung Tam Bao Ton Di Tich Quoc Gia Con Đao",
                    //    acqId = "970415",
                    //    addInfo =string.Format("muavethamquan DH{0}", madon),
                    ...
                    QRCodePaymentModel jsbody = new QRCodePaymentModel()
                    {
                        ...
                        accountName = "Huỳnh Trọng Nghĩa",
                        acqId = "970436", // VietCom
                        addInfo = string.Format("VQGCD{0}", madon),
                        amount = orderInfo.Total.ToString(),
                        template = "compact2",
                    };
        ...
        public IActionResult PaymentOrder(int? id)
        {
            ...
            var lstPaymentType = new List<ComboBoxModel>();
            lstPaymentType.Add(new ComboBoxModel() { Value = Contanst.PaymentType_TrucTiep, Text = "Chuyển khoản trực tiếp" });
            //lstPaymentType.Add(new ComboBoxModel() { Value = Contanst.PaymentType_OnePay, Text = "Thanh toán online (qua ngân hàng, ví điện tử, thẻ tín dụng...)" });
            ViewBag.LstPaymentType = lstPaymentType;
